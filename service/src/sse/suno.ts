		...
		proxyReqOptDecorator: function (proxyReqOpts, srcReq) {
			...
			if (process.env.SUNO_KEY) {
				proxyReqOpts.headers["Authorization"] =
					"Bearer " + process.env.SUNO_KEY;
			} else proxyReqOpts.headers["Authorization"] = "Bearer hi";
			proxyReqOpts.headers["Content-Type"] = "application/json";
			if (auth) {
				proxyReqOpts.headers["hk-me"] = auth.substring(0, 22);
			}
			return proxyReqOpts;
		},
...
export const sunoNewApiProxy = proxy(
	process.env.SUNO_NEWAPI_SERVER ?? "https://suno-api.suno.ai",
	{
		https: false,
		limit: "10mb",
		proxyReqPathResolver: function (req) {
		...
		proxyReqOptDecorator: function (proxyReqOpts, srcReq) {
			if (process.env.SUNO_NEWAPI_KEY)
				proxyReqOpts.headers["Authorization"] =
					"Bearer " + process.env.SUNO_NEWAPI_KEY;
			else proxyReqOpts.headers["Authorization"] = "Bearer hi";
			proxyReqOpts.headers["Content-Type"] = "application/json";
			return proxyReqOpts;
		},
...
//用户确认是否有积分
export const openHkUserCheck = async (
	request: Request,
	response: Response,
	next?: NextFunction
) => {
