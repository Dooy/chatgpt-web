...
export const checkWhileIp = async (
	uid: number,
	request: Request,
	...
	if (!mstr || !mvar || Object.keys(mvar).length == 0) {
		let res = await fetch(
			`${process.env.SSE_HTTP_SERVER}/openai/client/scheck/${uid}`
		);
...
export async function getMyKey(request: Request, enType: string): Promise<any> {
	...
	if (enType == "mj" && request.headers["mj-api-secret"] != undefined) {
		...
		authorization = (request.headers["mj-api-secret"] ??
	...
	if (!authorization || authorization == "")
		throw new mError("HK KEY ERROR, KEY缺失");
...
const getMyKeyDo = async (
	...
	const body = request.body;
	//mlog("log", "服务端获取用户信息>>", body);
	const kk = `hk:${arr[1]}`;
	let mvar: any = await redis.hGetAll(kk);
	//mlog("log", "服务端获取用户信息2>>", process.env.SSE_HTTP_SERVER);

	if (!mvar || Object.keys(mvar).length == 0 || !mvar.uid || +mvar.uid <= 0) {
		let res = await fetch(
			`${process.env.SSE_HTTP_SERVER}/openai/client/hk/${arr[1]}`
		);
		const rdate: any = await res.json();
		...
		if (hk) {
			await Object.keys(hk).map(async (k) => {
				await redis.hSet(kk, k, hk[k]);
			});
	...
	if (mvar.son) {
		...
		if (!attr || attr.hk_gpt3 == undefined) {
			let res = await fetch(
				`${process.env.SSE_HTTP_SERVER}/openai/client/hkopt/${mvar.uid}`
			);
			const rdate: any = await res.json();
...
//并发限制
const checkBingfa = async (
	mvar: any,
	redis: RedisClientType,
	model: string
) => {
	if (
		model.indexOf("gpt-4") > -1 ||
		model == "midjourney" ||
	...
	if (kv > lFen)
		throw new mError(
			`已触发低积分多请求限制(${lFen}RPM),请充值或者过1分钟后重试`
		);
};
...
const checkSon = (son: string, request: Request, enType: string) => {
	...
	try {
		...
		if (ajson.model && ajson.model.length > 0) {
			...
			let uModel = enType == "mj" ? "midjourney" : body.model;
			const isPorVideo = body.prompt && body.prompt.indexOf("--video") >= 0;
			const isUrlVideo = request.url && request.url.indexOf("video") > 0;
			if (enType == "mj" && (isPorVideo || isUrlVideo)) {
				//prompt
				//mlog("checkson body mj : ", body);
				uModel = "midjourney-video";
			}
			if (models.indexOf(uModel) == -1)
				throw new mError(`模型 ${uModel} 未开启！`);
		}
		...
		if (e instanceof mError) {
			let authorization = (request.headers["mj-api-secret"] ??
				request.headers["authorization"]) as string;
...
export const checkModelFotbitten = (model: string, attr: any) => {
	let forBitten = false;
	let is35 =
		model.indexOf("text-embedding") > -1 ||
		model.indexOf("tts") > -1 ||
		model.indexOf("whisper") > -1;
	if (model.indexOf("gpt-3.5") > -1 || is35) {
		...
		model.indexOf("gpt-4-all") > -1 ||
		model.indexOf("gpt-4-gizmo") > -1
	) {
...
async function whisperDo(
	...
	let tomq = {
		...
		request: { model: request.body.model, duration: 0 },
		response: "",
		reqid: clientId,
		status: 200,
		myKey: "",
	...
	let url = isNotEmptyString(process.env.SSE_API_BASE_URL)
		? process.env.SSE_API_BASE_URL
		...
		try {
			...
			try {
				...
				publishData(
					"openapi",
					"error",
					JSON.stringify({ e: { error: "获取时间识别 " }, tomq })
				);
				response
					.writeHead(428)
					.end(
						`{"error":{"message":"mp3 time error","type":"openai_hk_error","code":"mp3_time_error"}}`
					);
				return;
			}
...
export const sseDoTimeOut = async (
	request: Request,
	response: Response,
	next?: NextFunction
) => {
